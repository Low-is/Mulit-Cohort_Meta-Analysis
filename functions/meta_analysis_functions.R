...
generate_exprs_mtx <- function(DNA = NULL, RNA = NULL, dna_studies = list(), rna_studies = list()) {
  ...
  if (!is.null(DNA) && DNA == TRUE) {
    ...
    list_of_geo_data <- lapply(dna_studies, function(file) {
      tryCatch({
        gse_obj <- GEOquery::getGEO(GEO = file, AnnotGPL = TRUE, GSEMatrix = TRUE)
  ...
  if (!is.null(RNA) && RNA == TRUE) {
    ...
    get_rnaseq_matrix <- function(geo_id, outdir = tempdir()) {
      ...
      # --- Process COUNT FILES (ENSG / featureCounts) ---
      if (length(count_files) > 0) {
        dfs_count <- lapply(count_files, function(f) {
          dt <- data.table::fread(f)
          # featureCounts format: Geneid, Chr, Start, End, Strand, Length, sample.bam
          if ("Geneid" %in% colnames(dt)) {
            message("Detected featureCounts format in count file: ", basename(f))
            # Keep Geneid + last column (counts)
            sample_col <- colnames(dt)[ncol(dt)]
            dt_sub <- dt[, c("Geneid", sample_col), with = FALSE]
            colnames(dt_sub) <- c("gene", sub("\\.txt$", "", basename(f)))
          } else {
            # generic 2‑column case
            if (ncol(dt) == 2) {
              colnames(dt) <- c("gene", sub("\\.txt$", "", basename(f)))
              dt_sub <- dt
            } else {
              # first col gene, rest ignored (you never showed another pattern here)
              colnames(dt)[1] <- "gene"
      ...
      if (length(fpkm_files) > 0) {
        dfs_fpkm <- lapply(fpkm_files, function(f) {
          ...
          # Detect featureCounts output
          if ("Geneid" %in% colnames(dt) &&
              ncol(dt) > 1 &&
              any(grepl("\\.bam$", colnames(dt)[-1]))) {
      ...
      # ---------- CONDITIONAL ANNOTATION BLOCK ----------
      # Function to conditionally map ENSG→SYMBOL without ever returning 0 rows
      safe_annotate <- function(dt, geo_id) {
        if (is.null(dt) || nrow(dt) == 0) return(NULL)
        
        rn <- dt$gene
        id_no_ver <- sub("\\..*$", "", rn)
        ensg_like <- grepl("^ENSG", id_no_ver)
        frac_ensg <- mean(ensg_like)
        
        # If not mostly ENSG‑like, do not try to annotate
        if (is.na(frac_ensg) || frac_ensg <= 0.5) {
          message("Study ", geo_id, ": gene IDs not predominantly ENSG; skipping annotation, keeping original IDs.")
          dt$gene_symbol <- dt$gene
          return(dt)
        }
        
        message("Study ", geo_id, ": annotating ENSG IDs with org.Hs.eg.db")
        
        # Try mapping; if mapping yields nothing useful, keep ENSG IDs
        symbols <- tryCatch(
          AnnotationDbi::mapIds(
            x = org.Hs.eg.db,
            keys = id_no_ver,
          ...
          error = function(e) {
            message(sprintf(
              "mapIds ENSEMBL failed for RNA study %s: %s. Keeping ENSG IDs.",
              geo_id, e$message
            ))
            rep(NA_character_, length(id_no_ver))
          }
        )
        
        if (all(is.na(symbols))) {
          message("Study ", geo_id, ": no valid ENSEMBL→SYMBOL mappings; keeping ENSG IDs.")
          dt$gene_symbol <- dt$gene
          return(dt)
        }
        
        dt$gene_symbol <- symbols
        # keep rows with a symbol; if that drops everything, fall back
        keep <- !is.na(dt$gene_symbol) & dt$gene_symbol != ""
        if (!any(keep)) {
          message("Study ", geo_id, ": all mapped symbols NA/empty; keeping ENSG IDs.")
          dt$gene_symbol <- dt$gene
          return(dt)
        }
        ...
        expr_cols <- setdiff(colnames(dt_use), c("gene", "gene_symbol"))
        dt_agg <- dt_use[, lapply(.SD, mean, na.rm = TRUE), by = gene_symbol, .SDcols = expr_cols]
        # rename back to uniform column
        names(dt_agg)[1] <- "gene_symbol"
        dt_agg
      }
      # -------------------------------------------------
      
      # --- COMBINE: prefer count-derived, fallback to FPKM / featureCounts ---
      if (!is.null(expr_dt_count)) {
        message("Using matrix from count files for ", geo_id)
        # annotate count matrix if appropriate
        expr_dt_count_annot <- safe_annotate(expr_dt_count, geo_id)
        
        if (!is.null(expr_dt_count_annot) && "gene_symbol" %in% colnames(expr_dt_count_annot)) {
          expr_df <- as.data.frame(expr_dt_count_annot)
          rownames(expr_df) <- expr_df$gene_symbol
          expr_df$gene_symbol <- NULL
          if ("gene" %in% colnames(expr_df)) expr_df$gene <- NULL
        } else {
        ...
        message("Using matrix from FPKM / featureCounts files for ", geo_id)
        expr_dt_fpkm_annot <- safe_annotate(expr_dt_fpkm, geo_id)
        if (!is.null(expr_dt_fpkm_annot) && "gene_symbol" %in% colnames(expr_dt_fpkm_annot)) {
          expr_df <- as.data.frame(expr_dt_fpkm_annot)
          rownames(expr_df) <- expr_df$gene_symbol
          expr_df$gene_symbol <- NULL
          if ("gene" %in% colnames(expr_df)) expr_df$gene <- NULL
        } else {
      ...
      if (nrow(expr_df) == 0) {
        stop("Expression data for ", geo_id, " has 0 rows after processing; something is wrong with ID handling.")
      }
