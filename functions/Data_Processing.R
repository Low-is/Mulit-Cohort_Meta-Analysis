...
# Loading neonate sepsis datasets
sepsis_dna_neonate_files <- list(GSE25504 = "GSE25504",
                                 GSE69686 = "GSE69686")

sepsis_rna_neonate_files <- list(GSE138712 = "GSE138712")

# Loading pediatric sepsis datasets
sepsis_dna_pediatric_files <- list(GSE4607 = "GSE4607",
                                   GSE8121 = "GSE8121",
                                   GSE9692 = "GSE9692",
                                   GSE10361 = "GSE10361",
                                   GSE11755 = "GSE11755",
...
sepsis_rna_pediatric_files <- list(GSE279448 = "GSE279448")
...
add_column <- function(df,
                       name_column = "condition",
                       mutate_from = NULL,
                       ifelse = NULL,
                       condition = NULL,
                       new_condition = NULL) {
  pData_new <- df %>% 
    dplyr::mutate(name_column = ifelse(mutate_from == condition[1], new_condition[1], new_condition[2]))
...
# extract_pheno <- function(pdata_list, DNA = FALSE, RNA = FALSE) {
#   
#   if (DNA && !RNA) {
#     
#     columns_of_interest <- c(
#       "GA", "BW", "platform_id", "gsm", "source_name",
#       "study", "sex", "year", "title", "Sepsis_Status"
#     )
#     
#     purrr::imap(pdata_list, function(df, study_name) {
#       # require these core columns to exist
#       required <- c("platform_id", "gsm", "source_name", "study", "year", "title")
...
#       df_selected <- df %>%
#         dplyr::mutate(
#           Batch = match(study_name, names(pdata_list)),
#           Analysis_Type = NA_character_,
#           Total_samples = n()
#         )
#       
#       # add optional columns (GA, BW, sex, Sepsis_Status) if missing
#       optional <- setdiff(columns_of_interest, required)
#       missing_opt <- setdiff(optional, names(df_selected))
#       if (length(missing_opt) > 0L) {
#         for (mc in missing_opt) {
#           df_selected[[mc]] <- NA
#         }
#       }
#       
#       df_selected %>%
#         dplyr::select(
#           gsm, study, platform_id,
#           Analysis_Type, year, Batch,
#           Sepsis_Status, source_name, title,
#           Total_samples, GA, BW, sex
#         )
#     }) %>%
#       dplyr::bind_rows()
#   }
...
#     columns_of_interest <- c(
#       "GA", "BW", "platform_id", "gsm", "source_name",
#       "study", "sex", "year", "title", "Sepsis_Status", "condition"
#     )
#     
#     purrr::imap(pdata_list, function(df, study_name) {
#       # require these core columns to exist
...
#           gsm, study, platform_id,
#           Analysis_Type, year, Batch,
#           Sepsis_Status, condition, source_name, title,
#           Total_samples, GA, BW, sex
#         )
#     }) %>%
#       dplyr::bind_rows()
